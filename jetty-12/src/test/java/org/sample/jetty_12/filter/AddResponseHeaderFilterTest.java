 ...
 */
class AddResponseHeaderFilterTest {
  ...
  void servletWithFilter() throws Exception {
    ...
    servletContextHandler.addFilter(AddResponseHeaderFilter.class, HelloWorldServlet.SERVLET_PATH,
        dispatches);
    ...
    assertThat(response.headers().allValues(AddResponseHeaderFilter.CUSTOM_HEADER_KEY)).contains(
        "was-filtered");
    ...
    assertThat(response2.headers().allValues(AddResponseHeaderFilter.CUSTOM_HEADER_KEY)).contains(
        "was-filtered");
