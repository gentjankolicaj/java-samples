 ...
 */
@Slf4j
class MaxMessageSizeEchoEndpointTest extends WSTest {
  ...
  void websocketTest() throws LifecycleException, ConfigurationException,
    ...
    String websocketUri = MaxMessageSizeEchoEndpoint.WEBSOCKET_URI;

    //create websocket container
    WebSocketContainer wsContainer = ContainerProvider.getWebSocketContainer();
    //WS client config
    wsContainer.setDefaultMaxTextMessageBufferSize(1024); // Limit to 1 KB
    wsContainer.setDefaultMaxBinaryMessageBufferSize(1024); // Limit to 1 KB

    //create a client websocket session
    TestClientEndpoint testClientEndpoint = new TestClientEndpoint();
    Session session = wsContainer.connectToServer(testClientEndpoint,
  ...
  public static class TomcatWebSocketConfigImpl extends TomcatWebSocketConfig {
    ...
    {
      programmaticEndpoints = Set.of(new ProgrammaticEndpoint(MaxMessageSizeEchoEndpoint.class,
          MaxMessageSizeEchoEndpoint.WEBSOCKET_URI));
    }
  }
