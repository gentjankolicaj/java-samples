 ...
 * <br>For more check <a href="https://en.wikipedia.org/wiki/One-time_pad">OTP</a>
 * <br>Encryption : ( plaintext + key ) mod(charset-size) => ciphertext
 * <br>Decryption : ( ciphertext + key ) mod(charset-size) => ciphertext
 */
@Slf4j
public final class OTPCipher implements Cipher {
  ...
  public byte[] encrypt(byte[] plainText, Key key) throws CipherException {
    ...
    if (plainText.length > key.getEncoded().length) {
      throw new IllegalArgumentException("OTP cipher require plaintext length equal to key length");
    }
